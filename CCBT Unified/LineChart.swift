...
open class LineChart: UIView {
    ...
    public struct Coordinate {
        ...
        // private
        fileprivate var linear: LinearScaleTable!
    ...
    open var isScoreTypeProgress: Bool = false
    
    // values calculated on init
    fileprivate var drawingHeight: CGFloat = 0 {
        didSet {
            if isScoreTypeProgress{
                let max = getMaximumValue()
                let min = getMinimumValue()
                y.linear = LinearScaleTable(domain: [min, max], range: [0, drawingHeight])
    ...
    fileprivate var drawingWidth: CGFloat = 0 {
        didSet {
            if dataStore.count > 0{
                ...
                x.linear = LinearScaleTable(domain: [0.0, CGFloat(data.count - 1)], range: [0, drawingWidth])
    ...
    fileprivate var dotsDataStore: [[DotCALayerTable]] = []
    ...
    // category10 colors from d3 - https://github.com/mbostock/d3/wiki/Ordinal-Scales
        ...
        public var colors: [UIColor] = [
            UIColor(red: 0.1215690, green: 0.466667, blue: 0.705882, alpha: 1),
            UIColor(red: 1.0000000, green: 0.498039, blue: 0.054902, alpha: 1),
            UIColor(red: 0.1725490, green: 0.627451, blue: 0.172549, alpha: 1),
            UIColor(red: 0.8392160, green: 0.152941, blue: 0.156863, alpha: 1),
            UIColor(red: 0.5803920, green: 0.403922, blue: 0.741176, alpha: 1),
            UIColor(red: 0.5490200, green: 0.337255, blue: 0.294118, alpha: 1),
            UIColor(red: 0.8901960, green: 0.466667, blue: 0.760784, alpha: 1),
            UIColor(red: 0.4980390, green: 0.498039, blue: 0.498039, alpha: 1),
            UIColor(red: 0.7372550, green: 0.741176, blue: 0.133333, alpha: 1),
    ...
    override open func draw(_ rect: CGRect) {
        ...
        if isScoreTypeProgress {
             if y.labels.visible { drawYLabelsForScores() }
        }
        else{
        ...
        for (lineIndex, _) in dataStore.enumerated() {
            ...
            drawLine(lineIndex: lineIndex)
            
            // draw dots
            if dots.visible { drawDataDots(lineIndex: lineIndex) }
...
//        handleTouchEvents(touches as NSSet!, event: event!)
     ...
     */
        ...
        fileprivate func drawDataDots(lineIndex: Int) {
            let dotLayers: [DotCALayerTable] = []
     ...
     */
        ...
        fileprivate func drawLine(lineIndex: Int) {
     ...
     */
        fileprivate func drawXGrid() {
            ...
            let y2: CGFloat = 175//y.axis.inset
     ...
     */
        ...
        fileprivate func drawYGrid() {
            ...
            for i in stride(from: start, to : stop, by : step){
                ...
                let dashes: [CGFloat] = [path.lineWidth * 0, path.lineWidth * 6]
                path.setLineDash(dashes, count: dashes.count, phase: 0)
                path.lineCapStyle = CGLineCap.round
                UIGraphicsBeginImageContextWithOptions(CGSize(width:300,height: 20), false, 8)
                path.stroke()
     ...
     */
        ...
        fileprivate func drawXLabels() {
            ...
            for (index, _) in xAxisData.enumerated() {
                ...
                let label = UILabel(frame: CGRect(x: xValue, y: y, width: width, height: x.axis.inset+20))
     ...
     */
        ...
        fileprivate func drawYLabels() {
            ...
            var yLabels = [String]()
               ...
               yLabels = ["","", "1 - Anxious","","2", "","3", "","4","", "5 - Joyful", "\u{1F612}", "\u{1F611}", "\u{1F606}", "\u{1F612}","\u{1F606}", "\u{1F612}", "\u{1F611}", "\u{1F606}", "\u{1F612}","☎︎","","","",]
            
            let emoji_val :[String]=["","","emoji_1","","emoji_2","","emoji_3","","emoji_4","","emoji_5","","emoji_5","emoji_4","emoji_3","emoji_2","","emoji_1","","","","","","",]
    
    //        let reportLabel = []
            for i in stride(from:start , to :stop, by :step){
                yValue = self.bounds.height - self.y.scale(i) - (y.axis.inset * 1.5)
                let label = UILabel(frame: CGRect(x: -47, y: yValue, width: y.axis.inset+50, height: y.axis.inset))
                label.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2)
                label.textAlignment = .left
                label.text = yLabels[Int(i)]
                let labelEmoji = UIImageView(frame: CGRect(x: -65, y: yValue, width: y.axis.inset, height: y.axis.inset))
              //  labelEmoji.textAlignment = .Center
                labelEmoji.image = UIImage(named: emoji_val[Int(i)])
                self.addSubview(labelEmoji)
                self.addSubview(label)
            }
        }
    ...
    fileprivate func drawYLabelsForScores() {
        var yValue: CGFloat
        let (start, stop, step) = self.y.ticks
        
        for i in stride(from:start, to : stop, by : step){
            yValue = self.bounds.height - self.y.scale(i) - (y.axis.inset * 1.5)
            let label = UILabel(frame: CGRect(x: -10, y: yValue, width: y.axis.inset+50, height: y.axis.inset))
            label.font = UIFont.preferredFont(forTextStyle: UIFontTextStyle.caption2)
            label.textAlignment = .left
            label.text = "\(Int(i/step) * Int(ceil(stop/6.0)))"
            //  labelEmoji.textAlignment = .Center
